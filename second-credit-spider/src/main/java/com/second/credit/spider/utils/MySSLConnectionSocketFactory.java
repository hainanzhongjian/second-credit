 ...
 * against a list of trusted certificates and to authenticate to the HTTPS
 * server using a private key.
 * <p>
 * SSLSocketFactory will enable server authentication when supplied with a
 * {@link java.security.KeyStore trust-store} file containing one or several
 * trusted certificates. The client secure socket will reject the connection
 * during the SSL session handshake if the target HTTPS server attempts to
 ...
 * </pre>
 * <p>
 * In special cases the standard trust verification process can be bypassed by
 * using a custom {@link org.apache.http.conn.ssl.TrustStrategy}. This interface
 * is primarily intended for allowing self-signed certificates to be accepted as
 ...
 * {@link java.security.KeyStore key-store} file containing a private key/public
...
public class MySSLConnectionSocketFactory implements LayeredConnectionSocketFactory {
    private static final Logger logger = Logger.getLogger(MySSLConnectionSocketFactory.class);
    ...
    public static MySSLConnectionSocketFactory getSystemSocketFactory() throws SSLInitializationException {
        return new MySSLConnectionSocketFactory(
    ...
    public MySSLConnectionSocketFactory(final SSLContext sslContext, final String[] supportedProtocols,
    ...
    public MySSLConnectionSocketFactory(final javax.net.ssl.SSLSocketFactory socketfactory,
     ...
     * the SSL handshake happens). The default implementation is a no-op, but
     * could be overridden to, e.g., call
     * {@link SSLSocket#setEnabledCipherSuites(String[])}.
     */
    protected void prepareSocket(final SSLSocket socket) throws IOException { // NOSONAR
    }
